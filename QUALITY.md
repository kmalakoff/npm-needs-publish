...
const AUTH_MODE = requiredEnv('AUTH_MODE');
const config = AUTH_MODE === 'local'
  ? { key: requiredEnv('LOCAL_KEY') }
  : { key: requiredEnv('BROKER_KEY') };
```
...
- Conditional extraction based on configuration mode (AUTH_MODE pattern)
...
- Implement rate limiting for per-process scenarios in distributed environments
- Create retry logic for failure modes that should fail fast

## Recognition Signals
- Signal: Theoretical rate limiting implementations
  Examples: Per-process rate limiting in distributed systems, artificial quota enforcement below provider limits
  But also: Complex retry patterns for rate limit responses that should fail fast

- Signal: Defensive patterns without clear threat model
  Examples: Authentication rate limiting for AI agents with valid credentials
  But also: Quota retry logic when quotas reset daily
...
### Example 2: Theoretical Rate Limiting Violation
```typescript
// ❌ VIOLATION: Per-process rate limiting in distributed system
class TokenRateLimiter {
  private requests = new Map<string, number>();

  async checkLimit(clientId: string): Promise<boolean> {
    const count = this.requests.get(clientId) || 0;
    if (count > 60) return false; // Artificial limit, useless distributed
    this.requests.set(clientId, count + 1);
    return true;
  }
}
// VIOLATION REASON: Per-process limiting doesn't work in distributed systems
```

### Example 3: Inappropriate Retry Logic
```typescript
// ❌ VIOLATION: Retrying rate limits when quotas are daily
async function requestWithRetry(request: () => Promise<Response>): Promise<Response> {
  try {
    ...
    if (error.status === 429) {
      await delay(1000);
      return requestWithRetry(request); // Wrong - quotas don't reset quickly
    }
    throw error;
  }
}
// VIOLATION REASON: Should fail fast on 429, quotas are per day
```

### Example 4: Appropriate Network Retry
```typescript
// ✅ COMPLIANT: Retry network errors, fail fast on rate limits
async function requestWithBackoff(request: () => Promise<Response>): Promise<Response> {
  try {
    return await request();
  } catch (error) {
    if (error.status === 429) {
      throw error; // Fail fast - rate limits don't recover quickly
    }
    if (error.status >= 500) {
...
## Generalization Guidance
Ask yourself:
1. Does this defensive pattern address real threats in the deployment context?
2. Is this rate limiting effective in a distributed/multi-instance environment?
3. Should this error type fail fast or retry based on actual recovery characteristics?
...
## Generalization Guidance
Ask yourself:
1. Does this test check real service behavior or simulated behavior?
...
- Signal: Coverage-driven rather than value-driven testing
  ...
  But also: Tests that don't check meaningful behavior
...
2. Does this test check meaningful behavior rather than just achieving coverage?
3. Would this test provide value in catching real regressions?

## Anti-Patterns (What This Rule Does NOT Cover)
- Testing stable, public APIs that provide real value
- End-to-end tests that check complete workflows
